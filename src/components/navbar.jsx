...
export const Navbar = () => {
    ...
    const menuItem = [
        ...
        {id: '2', href: 'services', item: 'services'},
        {id: '3', href: 'works', item: 'works'},
        {id: '4', href: 'contact', item: 'contact'},
    ]
    ...
    return (
        <nav className="w-full fixed top-0 bg-white z-10 dark:bg-slate-900 shadow-xl rounded-lg">
            <div className="container mx-auto py-5 flex items-center justify-between">
                ...
                >
                    {menuItem.map(item => <li key={item.id} className="hover:text-gray-500 cursor-pointer">
                ...
                >
                    {menuItem.map(item => <li key={item.id}>
                        <Link id='hLink' to={item.href} onClick={handleHamburgerMenu} smooth={true}
                              duration={500}>{item.item}</Link>
                    </li>)}
                </ul>
            </div>
        </nav>
